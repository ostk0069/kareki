...
- Support for Dart 3.13 primary constructors, including declaring parameters,
  constructor bodies and initializer lists, field-initializer references, enum
  constant call sites, and concise `new` / `factory` declarations.

### Changed

- Raised the minimum supported analyzer version from 9.0.0 to 10.2.0 while
  retaining Dart 3.10 as the minimum supported SDK.
- Added CI coverage for both analyzer 10.2.0 and the latest analyzer allowed by
