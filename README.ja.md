...
**Dart / Flutter のワークスペース全体を対象としたデッドコード検出ツール**です。`dart analyze` が単一パッケージ内の private な未使用宣言しか検出しないのに対し、`kareki` は Melos / pub workspace 全体にまたがる参照を解決し、実プロジェクトに本当に溜まっていくデッドコード — 誰も呼んでいない public API、どこからも import されていないファイル、ぶら下がったままの pub 依存 — を洗い出します。
...
| ⚙️ | **CI 対応**。JSON 出力、決定的な終了コード、可搬な baseline。 |
...
詳細は [docs/baseline.ja.md](docs/baseline.ja.md)。
...
詳細は [docs/doctor.ja.md](docs/doctor.ja.md)。
