...
`kareki doctor` は、`kareki-config.yaml` の内容が現在のワークスペースの実態と整合しているかを検証します。何にもマッチしなくなった glob、すでに存在しないパッケージや依存を指している `ignore.*` のエントリ、もう何も抑制していない `// kareki: ignore_for_file=...` ディレクティブなど、現実から取り残された設定を浮き上がらせます。
...
| `unused-ignore-directive` | `// kareki: ignore_for_file=<rule>` ディレクティブが、そのファイルの実際の検出を何も抑制していない。 |
